...
export const addPost = () => {
  const lastId = state.profile.postData.slice(0)[0].id
...
export const sendMessage = () => {
  const lastId = state.dialogs.messagesData.slice(-1)[0].id
...
export const subscribe = (observer) => {
  //Патерн observer(по этому патерну работает addEventListener)
